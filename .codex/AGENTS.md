...
All project uses codebase-memory-mcp to maintain a knowledge graph of the codebase.
...
- Inspect the existing codebase and dependency manifests before writing custom parsing, validation, date handling, or state management. Prefer an appropriate maintained library already in the stack.
...
When several capabilities match, let the domain plugin own the workflow, use MCPs for evidence, `codebase-design` for interface decisions, and `tdd` for the implementation loop.
